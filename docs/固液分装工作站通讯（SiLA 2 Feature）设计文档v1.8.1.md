...
| 调试模块 | 夹爪开合（GripperTest）、夹爪旋转、夹爪归零、轴单步移动（AxisStepMove，电机物理坐标）、轴单步移动（AxisLogicalStepMove，逻辑坐标）、移液器归零 | 移液器气压、移液器工作状态 |
| 视觉模块 | 板位识别 | — |
...
| GripperParam | GripperParam | — | 夹爪参数（转移夹取时的张开宽度与夹持力矩） |
| ReleaseParam | GripperParam | — | 结束松夹爪参数；Position 与夹爪参数（GripperParam）Position 一致时表示结束后不松开夹爪 |
| Timeout | Integer | s | 命令执行超时时间，Client 缺省传 10 |
...
| OpenPosition | Position3D | mm | 开盖操作位置 |
| LidPlacePosition | Position3D | mm | 盖子放置位置 |
...
> Server 自动选择 **夹爪Z轴**。执行开盖流程（夹紧瓶身 → 旋开 → 放盖）。
...
|--------|------|------|------|
| ContainerDiameter | Real | mm | 容器直径（操作前用于夹紧瓶身，即挤压行程） |
| ClosePosition | Position3D | mm | 关盖操作位置（瓶位，独立参数） |
| LidPickPosition | Position3D | mm | 取盖位置（存盖位，关盖时夹爪先到此位置取盖） |
...
> Server 自动选择 **夹爪Z轴**。执行关盖流程（夹爪先到取盖位取盖 → 移至瓶位旋紧 → 释放夹爪）。
...
| v1.7 | 2026-09-09 | Debug：GripperRelease（夹爪松开）与 GripperGrasp（夹爪夹紧）入参完全相同（Speed/Position/Force/Timeout），合并为单一命令 GripperTest（夹爪开合）；删除了LED调试控制；FDL 文件版本保持 1.0 不变 | — |
| v1.8 | 2026-09-14 | Debug：新增 AxisLogicalStepMove（轴单步移动—逻辑坐标），与现有 AxisStepMove（轴单步移动—电机物理坐标）入参/返回完全一致（Axis / Speed / Position / Timeout → CommandResult），仅 Command Identifier 与语义不同（Server 按标定映射解算逻辑坐标→电机坐标后下发）；FDL 文件版本保持 1.0 不变 | — |
| v1.9 | 2026-09-16 | LidHandling：回退 v1.4 的 ToggleLid 合并，恢复为 OpenLid（开盖）与 CloseLid（关盖）两个独立命令；CloseLid 使用独立 ClosePosition 关盖操作位置与 LidPickPosition 取盖位置（关盖时夹爪先到取盖位取盖），ContainerDiameter（夹紧瓶身）两命令均保留；Transfer：TransferItem 入参 ReleaseAfterFinish（Boolean，是否松开夹爪）更名为 ReleaseParam（GripperParam 结构体：张开宽度 Position + 夹持力矩 Force），ReleaseParam.Position 与 GripperParam.Position 一致时表示结束后不松开夹爪；FDL 文件版本保持 1.0 不变 | — |
...
<Feature xmlns="http://www.sila-standard.org" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" Category="application" FeatureVersion="1.0" Originator="com.pharmablock" MaturityLevel="Draft" SiLA2Version="2.0">
    ...
    <Command>
        ...
        <Parameter>
            <Identifier>ReleaseParam</Identifier>
            <DisplayName>结束松夹爪参数</DisplayName>
            <Description>结束松夹爪参数；Position 与夹爪参数 Position 一致时表示不松开</Description>
            <DataType>
                <DataTypeIdentifier>GripperParam</DataTypeIdentifier>
            </DataType>
        </Parameter>
...
<Feature xmlns="http://www.sila-standard.org" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" Category="application" FeatureVersion="1.0" Originator="com.pharmablock" MaturityLevel="Draft" SiLA2Version="2.0">
    ...
    <!-- OpenLid -->
    <Command>
        <Identifier>OpenLid</Identifier>
        <DisplayName>开盖</DisplayName>
        <Description>执行开盖流程（夹紧瓶身→旋开→放盖）</Description>
        <Observable>Yes</Observable>
        <Parameter>
        ...
        <Parameter>
            ...
            <DisplayName>开盖操作位置</DisplayName>
            <Description>开盖操作位置</Description>
            <DataType>
                <DataTypeIdentifier>Position3D</DataTypeIdentifier>
            </DataType>
        </Parameter>
        <Parameter>
            ...
            <Description>盖子放置位置</Description>
            <DataType>
                <DataTypeIdentifier>Position3D</DataTypeIdentifier>
            </DataType>
        </Parameter>
    ...
    <!-- CloseLid -->
    <Command>
        <Identifier>CloseLid</Identifier>
        <DisplayName>关盖</DisplayName>
        <Description>执行关盖流程（夹爪先到取盖位取盖→移至瓶位旋紧→释放夹爪）</Description>
        <Observable>Yes</Observable>
        <Parameter>
            <Identifier>ContainerDiameter</Identifier>
            <DisplayName>容器直径(mm)</DisplayName>
        ...
        <Parameter>
            <Identifier>ClosePosition</Identifier>
            <DisplayName>关盖操作位置</DisplayName>
            <Description>关盖操作位置（瓶位，独立参数）</Description>
            <DataType>
                <DataTypeIdentifier>Position3D</DataTypeIdentifier>
            </DataType>
        </Parameter>
        <Parameter>
            <Identifier>LidPickPosition</Identifier>
            <DisplayName>取盖位置</DisplayName>
            <Description>取盖位置（存盖位，关盖时夹爪先到此位置取盖）</Description>
            <DataType>
                <DataTypeIdentifier>Position3D</DataTypeIdentifier>
            </DataType>
        </Parameter>
...
<Feature xmlns="http://www.sila-standard.org" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" Category="application" FeatureVersion="1.0" Originator="com.pharmablock" MaturityLevel="Draft" SiLA2Version="2.0">
    ...
    <Description>调试模式功能集合，包含夹爪调试、轴单步调试、移液器调试等功能</Description>
    ...
    <!-- GripperTest（原 GripperRelease / GripperGrasp 合并） -->
    <Command>
        <Identifier>GripperTest</Identifier>
        <DisplayName>夹爪开合</DisplayName>
        <Description>以指定速度、力矩将夹爪开合至指定位置</Description>
        <Observable>Yes</Observable>
        <Parameter>
    ...
    <!-- GripperHome -->
    <Command>
        <Identifier>GripperHome</Identifier>
        <DisplayName>夹爪归零</DisplayName>
        <Description>夹爪回零点</Description>
        <Observable>Yes</Observable>
        <Parameter>
            <Identifier>Timeout</Identifier>
            <DisplayName>超时时间(s)</DisplayName>
    ...
    <!-- AxisLogicalStepMove -->
    <Command>
        <Identifier>AxisLogicalStepMove</Identifier>
        <DisplayName>轴单步移动（逻辑坐标）</DisplayName>
        <Description>指定轴以指定速度单步移动到目标位置（逻辑坐标，Server 按标定映射解算为电机坐标后下发），axis取值：x / y / powder_z / liquid_z / gripper_z；入参/返回与 AxisStepMove 完全一致</Description>
        <Observable>Yes</Observable>
        <Parameter>
            <Identifier>Axis</Identifier>
            <DisplayName>轴标识</DisplayName>
        ...
        <Parameter>
            <Identifier>Position</Identifier>
            <DisplayName>目标位置(mm)</DisplayName>
            <Description>目标位置（逻辑坐标，mm）</Description>
            <DataType><Basic>Real</Basic></DataType>
        </Parameter>
        <Parameter>
            <Identifier>Timeout</Identifier>
