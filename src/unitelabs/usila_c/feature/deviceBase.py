...
class DeviceBaseFeature(sila.Feature):
    def __init__(self, uds: UdsClient):
        super().__init__(identifier="DeviceBase", name="DeviceBase", category="core", version="1.0")
    ...
    async def DeviceState(self) -> int:
        while True:
            try:
                ...
                resp = await uds.send_request(cmd="GetDeviceState", params={})
    ...
    async def FaultReason(self) -> str:
        while True:
            try:
                uds = await self._get_uds()
                resp = await uds.send_request(cmd="GetFaultReason", params={})
